...
const port = 19208;
const child = spawn(process.execPath, ['server/index.mjs'], { cwd: root, env: { ...process.env, KOTAUI_DATA_DIR: dataDir, KOTAUI_PORT: String(port), KOTAUI_SUBSCRIPTION_PORT: '19209', KOTAUI_PANEL_PATH: '/ptf', KOTAUI_ADMIN_USER: 'admin', KOTAUI_ADMIN_PASSWORD: 'test-password' }, stdio: 'ignore' });
try {
  ...
  const page = await fetch(`http://127.0.0.1:${port}/`, { redirect: 'manual' }); assert.equal(page.status, 302); assert.equal(page.headers.get('location'), '/ptf'); const panel = await fetch(`http://127.0.0.1:${port}/ptf`); assert.equal(panel.status, 200); assert.match(await panel.text(), /KotaUI/);
