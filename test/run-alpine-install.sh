...
printf '%s\n' '#!/bin/sh' 'set -eu' 'case "${1:-}" in --help) echo "  --ip-address IP_ADDRESS"; exit 0;; renew) exit 0;; esac' 'subject=""; ip=0; profile=""' 'while [ "$#" -gt 0 ]; do case "$1" in -d) subject="$2"; shift 2;; --ip-address) subject="$2"; ip=1; shift 2;; --preferred-profile) profile="$2"; shift 2;; *) shift;; esac; done' '[ -n "$subject" ] || exit 1' '[ "$ip" -eq 0 ] || [ "$profile" = shortlived ] || exit 1' 'mkdir -p "/etc/letsencrypt/live/$subject"' 'openssl req -x509 -newkey rsa:2048 -nodes -keyout "/etc/letsencrypt/live/$subject/privkey.pem" -out "/etc/letsencrypt/live/$subject/fullchain.pem" -subj "/CN=$subject" -days 7 >/dev/null 2>&1' > "$ROOTFS/usr/local/bin/certbot"
