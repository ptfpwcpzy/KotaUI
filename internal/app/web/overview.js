...
(() => {
  ...
  function chart() {
    ...
    const values = samples.map(s => s.avgMs).filter(Number.isFinite);
    ...
    pingData.targets.forEach((target,index)=>{if(hiddenTargets.has(target.id))return;const points=samples.filter(s=>s.targetId===target.id).sort((a,b)=>new Date(a.checkedAt)-new Date(b.checkedAt)).filter(s=>new Date(s.checkedAt)>=new Date(start));let path='',pen=false;points.forEach(s=>{const v=s.avgMs;if(!s.received||!Number.isFinite(v)){pen=false;return;}const point=`${x(s.checkedAt)},${y(v)}`;path+=(pen?' L':' M')+point;pen=true;});if(path)svg+=`<path d="${path}" fill="none" stroke="${colors[index%colors.length]}" stroke-width="2"/>`;});
    svg+='</svg>';return svg;
  }
  ...
  function renderNetwork(dashboard) {
    ...
    const legend=pingData.targets.map((t,i)=>`<button class="badge ${hiddenTargets.has(t.id)?'is-hidden':''}" data-nq-target="${t.id}"><i style="background:${colors[i%colors.length]}"></i>${window.esc(t.name)}</button>`).join('');
    card.innerHTML=`<div class="network-quality-head"><h2>网络质量</h2><p class="sub">最近 24 小时延迟 · 每分钟检测</p></div>${cards?`<div class="network-quality-targets"><div class="network-quality-table-head"><span>目标</span><span>平均延时</span><span>丢包</span><span>抖动</span></div>${cards}</div>`:'<div class="nq-empty">请在设置中添加 IP 或域名监测目标</div>'}${pingData.targets.length?chart():''}<div class="nq-legend">${legend}</div>`;
    card.querySelectorAll('[data-nq-target]').forEach(button=>button.onclick=()=>{const id=button.dataset.nqTarget;if(hiddenTargets.has(id))hiddenTargets.delete(id);else hiddenTargets.add(id);renderNetwork(dashboard)});
  }
