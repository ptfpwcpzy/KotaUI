...
(() => {
  ...
  function chart() {
    ...
    const values = samples.map(s => pingMode === 'loss' ? s.loss : pingMode === 'jitter' ? s.jitterMs : s.avgMs).filter(Number.isFinite);
  ...
  function renderNetwork(dashboard) {
    ...
    const latest=pingData.targets.map(t=>latestFor(t.id)).filter(Boolean);
    const average=latest.length?latest.reduce((sum,s)=>sum+s.avgMs,0)/latest.length:null;
    const loss=latest.length?latest.reduce((sum,s)=>sum+s.loss,0)/latest.length:null;
    const jitter=latest.length?latest.reduce((sum,s)=>sum+s.jitterMs,0)/latest.length:null;
    const metricCards=`<button class="nq-metric-card ${pingMode==='latency'?'active':''}" data-nq-mode="latency">延时<strong>${average===null?'--':average.toFixed(1)+' ms'}</strong></button><button class="nq-metric-card ${pingMode==='loss'?'active':''}" data-nq-mode="loss">丢包<strong>${loss===null?'--':loss.toFixed(2)+'%'}</strong></button><button class="nq-metric-card ${pingMode==='jitter'?'active':''}" data-nq-mode="jitter">抖动<strong>${jitter===null?'--':jitter.toFixed(2)+' ms'}</strong></button>`;
    card.innerHTML=`<div class="network-quality-head"><h2>网络质量</h2><p class="sub">最近 24 小时状态 · 每分钟检测</p></div>${cards?`<div class="network-quality-targets"><div class="network-quality-table-head"><span>目标</span><span>平均延时</span><span>丢包</span><span>抖动</span></div>${cards}</div>`:'<div class="nq-empty">请在设置中添加 IP 或域名监测目标</div>'}${pingData.targets.length?chart():''}<div class="nq-metric-cards">${metricCards}</div>`;
